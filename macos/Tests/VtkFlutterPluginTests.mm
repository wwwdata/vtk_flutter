...
- (void)addMethodCallDelegate:(id<FlutterPlugin>)delegate channel:(FlutterMethodChannel*)channel {
}
...
- (void)testTwoSessionsOwnIndependentViewLifecycles {
  ...
  XCTAssertEqualObjects(firstView[@"textureId"], @1);
  XCTAssertEqualObjects(secondView[@"textureId"], @2);
  ...
  XCTAssertEqualObjects(registrar.fakeTextures.unregisteredTextureIds, (@[ @1 ]));
  ...
  XCTAssertEqualObjects(registrar.fakeTextures.unregisteredTextureIds, (@[ @1, @2 ]));
  XCTAssertEqual(gDestroyedTargetSessions.size(), 3U);
}

- (void)testSameSessionCreateReusesItsTextureAndRejectsAnotherApi {
  FakePluginRegistrar* registrar = [[FakePluginRegistrar alloc] init];
  VtkFlutterPlugin* plugin = [[VtkFlutterPlugin alloc] initWithRegistrar:registrar];
  VtkFlutterPresentationApi api = PresentationApi();
  VtkFlutterPresentationApi otherApi = PresentationApi();
  auto* session = reinterpret_cast<VtkFlutterSession*>(static_cast<uintptr_t>(0x2800));
  ...
  NSDictionary* reusedView = [self invokePlugin:plugin
                                         method:@"createView"
                                      arguments:@{
                                        @"width" : @32,
                                        @"height" : @16,
  ...
  XCTAssertEqualObjects(firstView[@"textureId"], @1);
  XCTAssertEqualObjects(reusedView[@"textureId"], @1);
  XCTAssertEqual(gTargets.size(), 1U);
  XCTAssertEqual(registrar.fakeTextures.textures.count, 1U);

  FlutterError* error = [self invokePluginExpectingError:plugin
                                                   method:@"createView"
                                                arguments:@{
                                                  @"width" : @32,
                                                  @"height" : @16,
                                                  @"presentationApiAddress" : Address(&otherApi),
                                                  @"nativeSessionAddress" : Address(session),
                                                }];
  XCTAssertEqualObjects(error.code, @"invalid_state");
  XCTAssertEqual(gTargets.size(), 1U);
...
- (void)testRejectedTextureRegistrationRollsBackWithoutRetainingAView {
  ...
  XCTAssertEqualObjects(replacement[@"textureId"], @1);
  [self invokePlugin:plugin
              method:@"disposeView"
           arguments:@{@"nativeSessionAddress" : Address(session)}];
}
